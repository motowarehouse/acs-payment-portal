...
import { toNumber, formatEuro, formatDateTime } from '@/lib/utils'
...
export default async function DashboardPage() {
  ...
  const stats: Stat[] = [
    { key: 'outstanding', label: 'Outstanding COD', value: outstandingTotal, format: 'euro', accent: '#B7791F', icon: 'wallet' },
    { key: 'awaiting', label: 'Awaiting payment', value: awaitingCount, format: 'number', accent: '#009BB4', icon: 'clock' },
    { key: 'collected', label: 'Collected (all time)', value: toNumber(collectedAgg._sum.amount), format: 'euro', accent: '#2F8F5B', icon: 'check' },
    { key: 'exceptions', label: 'Needs review', value: exceptionCount + unmatchedCount, format: 'number', accent: '#DE1D1C', icon: 'alert' },
  ]
  ...
  return (
    <div>
      <PageHeader
        title="Dashboard"
        subtitle="ACS shipment and COD payment reconciliation"
        action={
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
        ...
        <div className="panel animate-fade-up" style={{ padding: 18 }}>
          <p style={{ fontSize: 13, fontWeight: 700, color: '#001A21', marginBottom: 14 }}>Shipment status</p>
          <StatusChart data={slices} />
        </div>

        {/* Today's reconciliation */}
        <div className="panel animate-fade-up" style={{ padding: 18 }}>
          <p style={{ fontSize: 13, fontWeight: 700, color: '#001A21', marginBottom: 4 }}>Today&apos;s reconciliation</p>
          <p style={{ fontSize: 11.5, color: '#8A939B', marginBottom: 14 }}>Payments recorded since midnight</p>
          <div style={{ display: 'flex', gap: 24, marginBottom: 16 }}>
            <div>
              <p style={{ fontSize: 30, fontWeight: 900, color: '#2F8F5B', lineHeight: 1 }}>{paidTodayAgg._count._all}</p>
              <p style={{ fontSize: 11, color: '#8A939B', textTransform: 'uppercase', letterSpacing: '0.06em', marginTop: 4 }}>payments</p>
            </div>
            <div>
              <p style={{ fontSize: 30, fontWeight: 900, color: '#001A21', lineHeight: 1 }}>{formatEuro(toNumber(paidTodayAgg._sum.amount))}</p>
              <p style={{ fontSize: 11, color: '#8A939B', textTransform: 'uppercase', letterSpacing: '0.06em', marginTop: 4 }}>collected today</p>
            </div>
          </div>
          <Link href="/outstanding" style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: 12.5, fontWeight: 700, color: '#009BB4', textDecoration: 'none' }}>
            View outstanding COD <ArrowRight size={13} />
          </Link>
        </div>
      </div>
      ...
      <div className="panel animate-fade-up" style={{ overflow: 'hidden' }}>
        <div style={{ padding: '14px 18px', borderBottom: '1px solid #EEF1F3', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <p style={{ fontSize: 13, fontWeight: 700, color: '#001A21' }}>Recent activity</p>
          <Link href="/import" style={{ fontSize: 12, color: '#009BB4', textDecoration: 'none', fontWeight: 700 }}>Import</Link>
        </div>
        {recentBatches.length === 0 ? (
          <div style={{ padding: '28px 18px', textAlign: 'center' }}>
            <p style={{ fontSize: 13, color: '#8A939B' }}>No imports yet. Head to Import to upload your first shipment file.</p>
          </div>
        ) : (
